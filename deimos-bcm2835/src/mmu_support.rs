...
pub unsafe fn set_mmu_enabled_features(config: MmuConfig) {
    let mut cr_on = 0;
    let mut cr_off = 0;
...
#[inline(never)]
unsafe fn init_mmu(ttb_ptr: *mut u32) {
    // Initialize TTB
    unsafe {
        // TTB
        init_mmu_translation_table(ttb_ptr);
        // DAC, PRRR, NMRR
        init_mmu_tex_remap();
    }
...
unsafe fn init_mmu_translation_table(ttb_ptr: *mut u32) {
...
const TT_SIZE: usize = 0x4000;
#[repr(C, align(0x4000))]
struct RawTT([u8; TT_SIZE]);
const _: () = assert!(align_of::<RawTT>() == TT_SIZE);
static TT: SyncUnsafeCell<RawTT> = SyncUnsafeCell::new(RawTT([0; TT_SIZE]));

pub unsafe fn init() {
    let tt_ptr = unsafe { TT.get().as_mut() }.unwrap().0.as_mut_ptr();
    unsafe { init_mmu(tt_ptr.cast()) };
}
