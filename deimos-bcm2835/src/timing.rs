...
unsafe fn __floating_time_unguarded(st: &SYSTMR) -> u64 {
    loop {
        let hi32 = st.chi().read().bits();
        let lo32 = st.clo().read().bits();
        if st.chi().read().bits() == hi32 {
            break ((hi32 as u64) << 32) | (lo32 as u64);
        }
    }
}
...
impl Instant {
    ...
    pub fn elapsed_from(&self, rhs: &Instant) -> Duration {
        Duration::from_micros(self.floating_micros.wrapping_sub(rhs.floating_micros))
    }
}
