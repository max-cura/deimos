...
// for <T: UIntLike>
macro_rules! impl_conversion_for {
    // XXX: LocalRegisterCopy::new is not unsafe, so it seems we should have From<uN> for LRC<uN, R>
    ($type:ty) => {
        ...
        impl<R: RegisterLongName> From<$type> for LocalRegisterCopy<$type, R> {
            fn from(r: $type) -> LocalRegisterCopy<$type, R> {
                LocalRegisterCopy::new(r)
            }
        }
    };
}

impl_conversion_for!(u8);
impl_conversion_for!(u16);
impl_conversion_for!(u32);
impl_conversion_for!(u64);
impl_conversion_for!(u128);
impl_conversion_for!(usize);
