...
use crate::{fields::FieldValue, ArrayDataType, DataType, ScalarDataType};
...
pub trait Modify: Register + Read + Write {
    fn modify(&self, field: FieldValue<<Self::DataType as DataType>::Value, Self::LongName>)
    where
        Self::DataType: ScalarDataType,
    {
        self.write(field.modify(self.read()))
    }
}
